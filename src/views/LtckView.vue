<template>
  <div class="wait">
    <header class="header">
      ...
      <BcxxCom
        v-if="!lt.isys && [2].includes(data?.wzzt)"
        :data="bcxx"
      />
      <div class="card">
        ...
        <div class="btns hui_6" v-if="lt.isys">
          ...
          <div class="ml cz" @click="bc.show=true" v-if="lt.isys && bc.data && bc.data.length">就诊信息</div>
          <div v-if="false">查病历夹</div>
          <van-popup v-if="lt.isys" v-model:show="bc.show" closeable position="bottom" :style="{ height: '100%',padding: '1rem 0.2rem 0' }">
        ...
        <div class="backhome" v-if="!lt.isys && +flag === 0" @click="tz('/index')">
          回到首页
        </div>
        <div class="backhome" v-if="!lt.isys && +flag === 1" @click="tz('/home')">
          回到首页
        </div>
    ...
    <div ref="view" class="view" @click="lt.hidden = false">
      <div class="lt">
        <div :class="+lt.guishu === +item.fsz ? 'mine ltxx' : 'other ltxx'" v-for="item in lt.aname" :key="item.id">
          ...
          <div class="tx">
            <img src="../assets/image/1.webp" v-if="!data.image && +data.ygdm === +item.fsz" alt="">
            <img :src="data.image" v-if="data.image && +data.ygdm === +item.fsz" alt="">
...
console.log(flag)
const titleid = route.query.titleId || route.query.titleid
const tpsc = ref(null)
const xx = reactive({
  data: {}
})
if (route.query.data) {
  xx.data = reactive(JSON.parse(route.query.data))
} else {
  proxy.$axios.post('/chat/getltbt', { openId: localStorage.getItem('token') }).then(d => {
    if (d.code === 1) {
      xx.data = d.data.find(i => +i.id === +titleid)
    }
  })
}

const { data } = toRefs(xx)
...
const lt = reactive({
  ...
  guishu: computed(() => localStorage.getItem('ysdm') || data.value.grxxid),
  hidden: false,
  fun: [
    {
    ...
    },
    {
      ...
      fn () {
        showConfirmDialog({
          ...
          .then(() => {
            proxy.$axios.post('/chat/endCaht', { titleId: data.value.id || titleid, status: 3 }).then(d => {
              if (!lt.isys && d.code === 1) {
                data.value.wzzt = 3
                router.replace({ path: '/ltck', query: { data: JSON.stringify(data) } })
              } else if (lt.isys && d.code === 1) {
                router.replace(`/yslb?ygdm=${data.value?.ygdm}&ysxm=${data.value?.ygxm}`)
              }
            })
          })
      }
  ...
  updchat () {
    proxy.$axios.post('/chat/getTheMessageList', { titleId: data.value.id || titleid }).then(d => {
      lt.aname = d.data
      lt.scrollbottom()
    })
  },
  ...
  fspj () {
    if (lt.message.trim()) {
      proxy.$axios.post('/chat/insertComment', {
        titleId: data.value.id || titleid,
        ysdm: data.value.ygdm,
        ysxm: data.value.ygxm,
        pjnr: lt.message,
        scoreNum: lt.star,
        commentName: data.value.name,
        openId: localStorage.getItem('token')
      }).then(d => {
        if (d.code === 1) {
          showToast('评论成功，谢谢配合')
          lt.showpj = false
          proxy.$axios.post('/chat/endCaht', { titleId: data.value.id || titleid, status: 4 }).then(d => {
            if (d.code === 1) {
              router.push('/index')
            }
          })
...
onMounted(() => {
  if (!lt.isys && data.value?.wzzt === 4) {
    proxy.$axios.post('/chat/getCommentOne', { titleId: data.value.id || titleid }).then(d => {
      lt.mymessage = d.data.pjnr
      lt.mystar = d.data.scoreNum
    })
  }
  lt.dsqId = setInterval(() => {
    if (data.value?.wzzt === 2) {
      lt.updchat()
    } else {
...
const bcxx = {
  openId: localStorage.getItem('token'),
  grxxid: data.value?.grxxid,
  btid: data.value?.id || titleid,
  wzzt: data.value?.wzzt
}
...
const bc = reactive({
  ...
  data: [],
  getxx () {
    proxy.$axios.post('/getczxx', bcxx).then(d => {
      bc.data = d.data
    })
  }
})
