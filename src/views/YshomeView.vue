  ...
  <div class="yshome">
    <header class="header">
      ...
      <div class="twwz shadow">
        ...
        <div class="middle">
          <div class="title">图文咨询<span class="red">￥0<i class="small">/次</i></span></div>
          <div class="small hui">使用图文咨询医生</div>
        </div>
        <div class="right">
          <div class="btn" @click="tz({path:'/bqms', query:{data:JSON.stringify(data)}})">立即咨询</div>
        </div>
      </div>
    </header>
    <main class="main">
      ...
      <div class="content">
        <div class="js" v-show="nav.index === 0">
          <div class="sc">
            ...
            <div class="nr hui_6">{{data.wzjs}}</div>
          </div>
          <div class="sc">
            <div class="title"><i class="clire"></i><span>擅长</span></div>
            <div class="nr hui_6">{{data.wzsc}}</div>
          </div>
          <div class="hzpj">
...
const data = JSON.parse(sessionStorage.getItem('ysxx'))
  ...
  showConfirmDialog({
    ...
    .then(() => {
      proxy.$axios.get('/getjzr', { openId: localStorage.getItem('token') }).then(d => {
        if (d.code === 1) {
          v.query.jzr = JSON.stringify(d.data)
          router.push(v)
        } else {
