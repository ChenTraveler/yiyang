...
import { showNotify, showToast, setNotifyDefaultOptions } from 'vant'
...
const { name, sfz, sex, szd, xxdz, lxdh, mz, gx, jzk } = toRefs(form)
...
if (data) {
  for (const k in form) {
    if (k === 'sex') {
      sexobj.sex = data.sex === 0 ? '女' : '男'
      sex.value = data.sex
    } else {
      data[k] && (form[k] = data[k])
    }
  }
}
...
const onSubmit = () => {
  if (!isnull(form)) {
    ...
    if (jzk.value) {
      proxy.$axios.post('/bdjzr', { jzk: jzk.value, name: name.value }).then(d => {
        if (d.code === 1) {
          form.brid = d.data
          if (type === 'bq') {
            form.id = data.id
          ...
          showNotify({ type: 'primary', message: d.msg })
        }
      })
    } else {
      if (type !== 'bq') {
